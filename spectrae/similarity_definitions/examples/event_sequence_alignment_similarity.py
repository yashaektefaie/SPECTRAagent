#!/usr/bin/env python3
"""Create pairwise_similarity.csv from token-level global alignment identity."""
...
def parse_tokens(value, delimiter):
    if delimiter == "":
...
def main():
    ...
    parser.add_argument("--sequence-col", default="events")
    ...
    train_sequences = [
        (row[args.train_id_col], parse_tokens(row[args.sequence_col], args.delimiter))
    ...
    with open(args.out, "w", newline="", encoding="utf-8") as handle:
        ...
        for eval_row in eval_rows:
            eval_sequence = parse_tokens(eval_row[args.sequence_col], args.delimiter)
            for train_id, train_sequence in train_sequences:
                writer.writerow(
                    {
